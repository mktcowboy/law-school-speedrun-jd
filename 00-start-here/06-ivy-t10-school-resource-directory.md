# Ivy/T10 Resource Directory

Verified and expanded on `June 2, 2026`.

This directory gathers `free`, `public`, `school-produced` resources from Ivy League and other `T10` law schools that fit this curriculum.

Use the main year guides first. Use this directory when you want to deepen a subject, improve your research process, or write more like a real law student.

## How To Use This Directory

- use the `1L` and `2L` reading guides for your main sequence
- use this directory when you want a stronger `research`, `writing`, or `subject-deepening` layer
- prefer `official law` whenever the real task is reading the controlling rule, statute, or constitutional text
- treat `Stanford` as first-line support whenever you need `free legal research`, `docket`, `Supreme Court`, `legislative history`, or `transactional` guidance

## Harvard

### Best For

- open casebooks
- first-pass doctrinal reading
- beginner legal research
- statutory research
- legal history and source tracing

### Core Resources

- H2O Open Casebooks: <https://opencasebook.org/>
- H2O featured casebooks: <https://opencasebook.org/featured/>
- Legal Research Strategy: <https://guides.library.harvard.edu/law/researchstrategy>
- Statutes: U.S. and State Codes: <https://guides.library.harvard.edu/law/statutes>
- Online Legal History Sources: <https://guides.library.harvard.edu/onlinelegalhistory>
- CopyrightX (William Fisher, HarvardX) free lectures: <https://copyx.org/lectures/>
- CopyrightX course home: <https://copyx.org/>
- Justice (Michael Sandel) free course and videos: <https://justiceharvard.org/>

### Best Uses In This Curriculum

- start `1L` doctrinal reading here
- use the statutes guide when `contracts`, `commercial law`, or `administrative law` become code-heavy
- use the legal-history guide when constitutional materials need historical context
- use `CopyrightX` as a full intellectual-property elective if you go that direction in `2L/3L`
- use `Justice` (Sandel) as a free moral- and political-philosophy companion for `criminal law`, jurisprudence, and constitutional-rights reasoning
- if you can access it, treat Harvard's `Zero-L` pre-orientation course as a model for the methods bridge in [00-pre-1l-methods-and-mindset](../00-pre-1l-methods-and-mindset/README.md)

## Yale

### Best For

- foundational constitutional materials
- research workflow
- dockets, briefs, and oral arguments
- substantial writing projects

### Core Resources

- Yale Law Library research guides hub: <https://library.law.yale.edu/research/research-guides>
- Avalon Project: <https://avalon.law.yale.edu/>
- Constitution Day Resources: <https://library.law.yale.edu/research-guides/constitution-day-resources>
- Docket Research: <https://library.law.yale.edu/research-guides/docket-research>
- Locating Court Records, Briefs and Oral Arguments: <https://library.law.yale.edu/research-guides/locating-court-records-briefs-and-oral-arguments>
- Guide for Substantial and Supervised Analytical Writing Research: <https://library.law.yale.edu/research-guides/guide-substantial-and-supervised-analytical-writing-saw-research>
- Akhil Amar (Yale) "Amarica's Constitution" podcast: <https://amaricasconstitution.podbean.com/>
- Akhil Amar faculty site and videos: <https://akhilamar.com/>
- Yale Law School announcement of the podcast: <https://law.yale.edu/yls-today/news/professor-amar-launches-podcast-constitutional-issues>

### Best Uses In This Curriculum

- use `Avalon` and `Constitution Day Resources` to deepen constitutional law
- use `Docket Research` and `Locating Court Records, Briefs and Oral Arguments` when you want to see how cases look outside trimmed casebook excerpts
- use the `SAW` guide when you begin seminar-paper or capstone-level research
- use `Akhil Amar`'s podcast and videos for Yale constitutional structure, history, and method

## Stanford

### Best For

- free and low-cost legal research
- case-finding and search strategy
- dockets and Supreme Court materials
- legislative history
- transactional and deal research
- note and comment writing

### Core Resources

- Stanford law-library resources hub: <https://guides.law.stanford.edu/libraryresources>
- Free and Low-Cost Legal Research: <https://guides.law.stanford.edu/free-lowcost>
- Case Finding and Advanced Searching Strategies: <https://guides.law.stanford.edu/cases>
- Docket Research: <https://guides.law.stanford.edu/dockets>
- Researching and Locating Supreme Court Materials: <https://guides.law.stanford.edu/supremecourt>
- Federal Legislative History Research: <https://guides.law.stanford.edu/fedleghistory>
- Transactional Law Research: <https://guides.law.stanford.edu/transactionallaw>
- Writing Notes and Comments for Journal Members: <https://guides.law.stanford.edu/writingnotes>

### Best Uses In This Curriculum

- use Stanford when you want a more disciplined `free-first` research workflow before touching paid databases
- use the `Cases`, `Dockets`, and `Supreme Court Materials` guides to deepen `civil procedure`, `criminal procedure`, `constitutional law`, and appellate reading
- use `Federal Legislative History Research` when `administrative law`, `statutory interpretation`, or other code-heavy work turns ambiguous
- use `Transactional Law Research` when `business associations` or `commercial law` starts moving from doctrine into filings, company materials, and deal structure
- use `Writing Notes and Comments` when `2L/3L` writing becomes longer, more source-heavy, and closer to publishable scholarship

## Columbia

### Best For

- legal writing
- memo editing
- speech and free-expression law
- corporate governance and capital-markets reading

### Core Resources

- Writing Center: <https://www.law.columbia.edu/academics/experiential/legal-writing/writing-center>
- Legal Writing Resources: <https://www.law.columbia.edu/academics/experiential/legal-writing/writing-center/legal-writing-resources>
- Memo Writing Checklist PDF: <https://www.law.columbia.edu/sites/default/files/2022-08/WC%20Memo%20Checklist.pdf>
- Global Freedom of Expression database: <https://globalfreedomofexpression.columbia.edu/>
- Teaching Freedom of Expression Without Frontiers: <https://teaching.globalfreedomofexpression.columbia.edu/>
- CLS Blue Sky Blog: <https://clsbluesky.law.columbia.edu/>

### Best Uses In This Curriculum

- use Columbia's handouts to improve `1L` memo writing and `2L/3L` analytical prose
- use `Global Freedom of Expression` and the teaching portal to deepen `First Amendment` work
- use `Blue Sky` to keep `business associations`, `corporate law`, and `securities-related` reading tied to current debates

## Penn

### Best For

- legal research foundations
- statutory research
- administrative and regulatory law

### Core Resources

- Biddle Law Library research page: <https://www.law.upenn.edu/library/research.php>
- Legal Research Fundamentals: <https://law.upenn.libguides.com/researchfundamentals>
- Statutory Research page: <https://law.upenn.libguides.com/researchfundamentals/statutes>
- The Regulatory Review: <https://www.theregreview.org/>
- About The Regulatory Review: <https://www.theregreview.org/about-us/>

### Best Uses In This Curriculum

- use `Legal Research Fundamentals` as another excellent `1L` research map
- use the statutory-research page when working through statutes in `contracts`, `commercial law`, and `admin law`
- use `The Regulatory Review` as a reading companion for `administrative law` and regulatory-state topics

## Duke

### Best For

- law-library research guides
- court rules
- business associations
- UCC and commercial law
- administrative law
- legal ethics

### Core Resources

- Duke research guides hub: <https://law.duke.edu/lib/research-guides>
- Court Rules guide: <https://law.duke.edu/lib/research-guides/court-rules>
- Business Associations guide: <https://law.duke.edu/lib/research-guides/business-associations/>
- Uniform Commercial Code guide: <https://law.duke.edu/lib/research-guides/ucc/>
- Federal Administrative Law guide: <https://law.duke.edu/lib/research-guides/federal-administrative-law/>
- Legal Ethics guide: <https://law.duke.edu/lib/research-guides/legal-ethics/>

### Best Uses In This Curriculum

- use `Court Rules` when you want stronger procedural-source discipline in `civil procedure` or `evidence`
- use the `Business Associations` and `UCC` guides to structure upper-level code and entity reading
- use `Federal Administrative Law` to understand the full agency materials ecosystem
- use `Legal Ethics` when professional responsibility questions become jurisdiction-specific or opinion-heavy

## NYU

### Best For

- substantial legal writing
- note writing
- upper-level writing development

### Core Resources

- Writing a Student Note: <https://www.law.nyu.edu/students/studentwriting>
- NYU Resources on Writing PDF: <https://www.law.nyu.edu/sites/default/files/Resources%20on%20writing%20-%20accessible.pdf>
- Journals and Publications: <https://www.law.nyu.edu/journals>

### Best Uses In This Curriculum

- use NYU's note-writing material when you move from class outlines into sustained legal scholarship
- use the writing PDF when planning `2L/3L` seminar papers, journal notes, or capstone writing

## Essential Free Backbone Beyond The Top-10 Band

These are nonprofit and public-service standards that no top-10 school replicates. Under the sourcing rule, the top-10 band governs `teaching materials`; for `primary law`, `reference`, and `practice tools`, use the best-in-class free host regardless of rank. See [05-top-10-school-priority-basis.md](./05-top-10-school-priority-basis.md).

### Cornell Legal Information Institute (LII)

### Best For

- free primary law and quick doctrinal reference

### Core Resources

- LII home: <https://www.law.cornell.edu/>
- Wex legal dictionary and encyclopedia: <https://www.law.cornell.edu/wex>
- U.S. Code: <https://www.law.cornell.edu/uscode/text>
- U.S. Constitution: <https://www.law.cornell.edu/constitution>
- Federal Rules collection: <https://www.law.cornell.edu/rules>
- Federal Rules of Civil Procedure: <https://www.law.cornell.edu/rules/frcp>
- Federal Rules of Evidence: <https://www.law.cornell.edu/rules/fre>

### CALI And eLangdell

### Best For

- guided, auto-scored practice lessons and free open casebooks

### Core Resources

- CALI home (lessons; school authorization code required): <https://www.cali.org/>
- eLangdell open casebooks: <https://www.cali.org/the-elangdell-bookstore>
- About eLangdell: <https://www.cali.org/elangdell/about>

### National Constitution Center

### Best For

- nonpartisan, clause-by-clause constitutional reference

### Core Resources

- Interactive Constitution (about): <https://constitutioncenter.org/the-constitution/about-the-interactive-constitution>
- National Constitution Center constitution hub: <https://constitutioncenter.org/the-constitution>

Note: the Interactive Constitution pairs scholars selected with the `American Constitution Society` and the `Federalist Society`, which is why it reads as balanced rather than one-sided.

## Best Pairings By Subject

### Civil Procedure

- Harvard H2O Civil Procedure
- Duke Court Rules
- Yale Docket Research
- Stanford Docket Research
- Yale Briefs and Oral Arguments guide

### Constitutional Law

- Yale Avalon
- Yale Constitution Day Resources
- Akhil Amar (Yale) podcast and videos
- National Constitution Center Interactive Constitution
- Stanford Supreme Court Materials
- Columbia Global Freedom of Expression
- Columbia Teaching Portal

### Legal Research And Writing

- Harvard Legal Research Strategy
- Harvard Statutes guide
- Stanford Free and Low-Cost Legal Research
- Stanford Case Finding and Advanced Searching Strategies
- Yale SAW guide
- Penn Legal Research Fundamentals
- Columbia writing handouts
- Stanford Writing Notes and Comments
- NYU note-writing materials

### Business Associations

- Harvard-hosted H2O if you want a casebook spine
- Duke Business Associations guide
- Stanford Transactional Law Research
- Delaware Code
- Columbia CLS Blue Sky Blog

### Commercial Law And UCC

- Duke UCC guide
- Harvard statutes guide
- Penn statutory-research page
- Stanford Transactional Law Research
- Uniform Law Commission materials

### Administrative Law

- Harvard-hosted H2O Administrative Law
- Penn The Regulatory Review
- Stanford Federal Legislative History Research
- Duke Federal Administrative Law guide

### Professional Responsibility

- ABA Model Rules
- Harvard-hosted H2O Professional Responsibility
- Duke Legal Ethics guide

### Intellectual Property (Elective)

- Harvard CopyrightX lectures (William Fisher)
- Harvard-hosted H2O for any IP casebook spine
- U.S. Copyright Office and USPTO primary materials

### Criminal Law And Jurisprudence

- Harvard-hosted H2O Criminal Law
- Harvard Justice (Sandel) for moral and political theory behind punishment
- the Model Penal Code text itself for the mental-state framework

## Practical Rule

If you are unsure which `Ivy/T10` resource to use:

- use `Harvard` for casebooks and first-pass doctrinal reading
- use `Yale` for constitutional and research-process deepening
- use `Stanford` for free-research strategy, dockets, Supreme Court materials, legislative history, and transactional-research infrastructure
- use `Columbia` for writing and speech resources
- use `Penn` for regulatory and statutory research framing
- use `Duke` for research-guide architecture in upper-level subjects
- use `NYU` for substantial writing and note-writing development
